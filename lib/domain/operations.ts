export type TripStatus = 'draft' | 'scheduled' | 'assigned' | 'driver_en_route' | 'at_pickup' | 'in_progress' | 'completed' | 'cancelled'
export type AssignmentStatus = 'unassigned' | 'pending' | 'assigned' | 'declined' | 'cancelled'
export type ServiceType = 'aeropool' | 'private' | 'charter' | 'other'
...
const tripStatuses: readonly TripStatus[] = ['draft','scheduled','assigned','driver_en_route','at_pickup','in_progress','completed','cancelled']
const assignmentStatuses: readonly AssignmentStatus[] = ['unassigned','pending','assigned','declined','cancelled']
const serviceTypes: readonly ServiceType[] = ['aeropool','private','charter','other']
...
export function parseTrip(id: string, value: unknown): Trip {
  ...
  return { id, status: oneOf(data.status, tripStatuses, 'draft'), assignmentStatus: oneOf(assignment.status ?? data.assignmentStatus, assignmentStatuses, 'unassigned'), serviceType: oneOf(data.serviceType, serviceTypes, 'other'), airport: text(data.airport), terminal: text(data.terminal), pickupZone: text(data.pickupZone), pickupAt: asDate(data.pickupAt), requiredArrivalAt: asDate(data.requiredArrivalAt), createdAt: asDate(data.createdAt), updatedAt: asDate(data.updatedAt), crew, pickupStops, operator: text(operatorData.id) ? { id: text(operatorData.id)!, name: text(operatorData.name), companyName: text(operatorData.companyName), status: text(operatorData.status) } : null, driverName: text(assignment.driverName ?? data.driverName), vehicle: text(vehicleData.id) ? { id: text(vehicleData.id)!, registration: text(vehicleData.registration), type: text(vehicleData.type), capacity: number(vehicleData.capacity) } : null, paymentStatus: data.paymentStatus ? oneOf(data.paymentStatus, paymentStatuses, 'pending') : null, incidentIds: stringArray(data.incidentIds), auditEventIds: stringArray(data.auditEventIds) }
}

export function parseRequirement(id: string, value: unknown): TransportRequirement { const d=record(value); return { id, tripId:text(d.tripId), assignmentStatus:oneOf(d.assignmentStatus,assignmentStatuses,'unassigned'), serviceType:oneOf(d.serviceType,serviceTypes,'other'), pickupZone:text(d.pickupZone), pickupAt:asDate(d.pickupAt), requiredArrivalAt:asDate(d.requiredArrivalAt), airport:text(d.airport), terminal:text(d.terminal), crewCount:number(d.crewCount) ?? 0, createdAt:asDate(d.createdAt) } }
...
export const tripStatusLabel = (status: TripStatus) => ({ draft:'Draft', scheduled:'Scheduled', assigned:'Assigned', driver_en_route:'Driver en route', at_pickup:'At pickup', in_progress:'In progress', completed:'Completed', cancelled:'Cancelled' })[status]
